import streamlit as st
import cv2
import numpy as np
from tensorflow.keras.models import load_model
from PIL import Image

# --- Konfigurasi dan Pemuatan Model ---
st.set_page_config(page_title="Deteksi Emosi Wajah", layout="centered")

@st.cache_resource
def load_emotion_model():
    # Menggunakan compile=False mempercepat pemuatan dan meningkatkan stabilitas
    model = load_model("emotion_model.h5", compile=False)
    return model

@st.cache_resource
def load_face_cascade():
    # Memuat cascade di cache juga
    return cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')

emotion_model = load_emotion_model()
face_cascade = load_face_cascade()

ORIGINAL_LABELS = ['Angry', 'Disgust', 'Fear', 'Happy', 'Neutral', 'Sad', 'Surprise']

def map_emotion(emotion):
    if emotion in ['Happy', 'Surprise']:
        return 'satisfied'
    elif emotion in ['Angry', 'Disgust', 'Fear', 'Sad']:
        return 'unsatisfied'
    else:
        return 'neutral'

# --- Fungsi untuk Memproses Gambar (Reusable) ---
def process_image(frame_bgr):
    """Fungsi ini mengambil frame BGR dari OpenCV dan mengembalikan frame dengan deteksi."""
    gray = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2GRAY)
    faces = face_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30))

    if len(faces) == 0:
        st.warning("Tidak ada wajah yang terdeteksi.")
        return frame_bgr

    st.success(f"Terdeteksi {len(faces)} wajah!")
    for (x, y, w, h) in faces:
        # Gambar kotak di sekitar wajah
        cv2.rectangle(frame_bgr, (x, y), (x + w, y + h), (255, 0, 0), 2)
        
        # Persiapan ROI untuk prediksi
        roi_gray = gray[y:y + h, x:x + w]
        roi_gray = cv2.resize(roi_gray, (48, 48))
        roi_gray = np.expand_dims(roi_gray, axis=0)
        roi_gray = np.expand_dims(roi_gray, axis=-1) / 255.0

        # Lakukan prediksi
        prediction = emotion_model.predict(roi_gray, verbose=0)[0]
        max_index = np.argmax(prediction)
        
        predicted_emotion = ORIGINAL_LABELS[max_index]
        final_emotion = map_emotion(predicted_emotion)
        
        label = f"{final_emotion}: {prediction[max_index]*100:.2f}%"
        cv2.putText(frame_bgr, label, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)

    return frame_bgr

# --- Antarmuka Pengguna (UI) Streamlit ---
st.title("🤖 Deteksi Emosi Wajah")
st.write("Aplikasi ini dapat mendeteksi emosi 'satisfied', 'unsatisfied', atau 'neutral' dari wajah melalui unggahan gambar atau snapshot webcam.")

# PERUBAHAN ADA DI BARIS INI
mode = st.sidebar.selectbox(
    "Pilih Mode:",
    ["🖼️ Unggah Gambar", "📸 Ambil Gambar dari Webcam"],
    key="mode_selector"  # Kunci unik untuk widget
)

if mode == "📸 Ambil Gambar dari Webcam":
    st.header("Deteksi dari Snapshot Webcam")
    st.info("Klik tombol di bawah untuk mengambil gambar dari webcam Anda dan menganalisisnya.")

    if st.button("Ambil Gambar"):
        camera = cv2.VideoCapture(0)
        if not camera.isOpened():
            st.error("Tidak dapat membuka kamera. Pastikan kamera tidak digunakan oleh aplikasi lain.")
        else:
            ret, frame = camera.read()
            camera.release()
            if not ret:
                st.error("Gagal mengambil gambar dari kamera.")
            else:
                st.success("Gambar berhasil diambil!")
                processed_frame = process_image(frame)
                processed_frame_rgb = cv2.cvtColor(processed_frame, cv2.COLOR_BGR2RGB)
                st.image(processed_frame_rgb, caption='Hasil Deteksi', use_column_width=True)

elif mode == "🖼️ Unggah Gambar":
    st.header("Analisis Emosi dari Gambar")
    uploaded_file = st.file_uploader("Pilih sebuah gambar...", type=["jpg", "jpeg", "png"])

    if uploaded_file is not None:
        image = Image.open(uploaded_file).convert('RGB')
        frame_rgb = np.array(image)
        frame_bgr = cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2BGR)
        st.image(image, caption='Gambar yang Diunggah', use_column_width=True)
        processed_frame = process_image(frame_bgr)
        processed_frame_rgb = cv2.cvtColor(processed_frame, cv2.COLOR_BGR2RGB)
        st.image(processed_frame_rgb, caption='Hasil Deteksi', use_column_width=True)